...
Accurate Cobb angle measurement is fundamental to scoliosis assessment and follow-up. However, standing radiographs with broad anatomical coverage contain non-spinal osseous structures, such as the skull and pelvis, whose local responses resemble those of vertebrae and interfere with vertebral localization and subsequent angle estimation. We propose a structured multiscale sparse decoding framework that relates local osseous responses to the continuous spinal structure during feature formation. Specifically, a predicted centerline guides spine-level coarse localization, establishing spatial constraints for subsequent vertebra-level decoding. Building on this guidance, a multiscale vertebral-chain representation integrates longitudinal vertebral arrangement with high-resolution local geometry, while sparse attention selects row tokens for aggregation based on structural support and propagates context along the spinal axis. This coarse-to-fine organization combines global spinal context with local geometric evidence before predicting vertebral centers and corners for Cobb angle computation. Experiments on AASCE-128, AASCE-98, and the external Clinical-150 dataset show that the proposed method achieves the lowest point localization error among the reproduced methods on all three datasets. On AASCE-98, the Cobb angle mean absolute error is 1.96° and the symmetric mean absolute percentage error is 7.49%; the corresponding errors on Clinical-150 are 2.13° and 6.74%, with a point localization error of 20.51 px. These results demonstrate that continuous spinal guidance within multiscale decoding improves vertebral landmark localization and Cobb angle measurement in radiographs with broad anatomical coverage.
